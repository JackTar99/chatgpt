  ...
  function updateGame() {
    ...
    if (gameStatus === "playing") {
      ...
      for (var m = 0; m < missiles.length; m++) {
        context.drawImage(missileImage, missiles[m].x, missiles[m].y, missiles[m].width, missiles[m].height);
      }

      // Draw the invaders
      for (var n = 0; n < invaders.length; n++) {
        context.drawImage(invaderImage, invaders[n].x, invaders[n].y, invaders[n].width, invaders[n].height);
      }
